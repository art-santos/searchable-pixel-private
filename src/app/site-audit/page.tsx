    ...
    <div className={`relative`} style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span
          ...
          style={{ fontSize: Math.max(16, size * 0.25) }}
        >
          {score}
        </span>
        <span
...
export default function SiteAudit() {
  ...
  const [performInteractiveActions, setPerformInteractiveActions] =
    useState(false);
  ...
  const [lastCrawlIdForAiRec, setLastCrawlIdForAiRec] = useState<string | null>(
    null,
  );

  // State for crawl history
  const [crawlHistory, setCrawlHistory] = useState<HistoricalCrawl[] | null>(
    null,
  );
  const [isLoadingHistory, setIsLoadingHistory] = useState(true);

  // In SiteAudit component, add state for the typewriter effect
  const [displayedAiRecommendations, setDisplayedAiRecommendations] =
    useState<string>("");
  ...
  const fetchHistory = async () => {
    ...
    try {
      const response = await fetch("/api/aeo-scorecard/history");
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    if (
      crawlStartTime &&
      Date.now() - crawlStartTime > 120000 &&
      !showTimeoutWarning
    ) {
      ...
      toast({
        ...
        description:
          "The website may be blocking our crawler or is very slow to respond. You may want to try a different site.",
        variant: "destructive",
        duration: 10000,
      });
    }
    ...
    const statusInterval = setInterval(async () => {
      try {
        ...
        if (data.totalPages && data.processedPages != null) {
          const pct = Math.round((data.processedPages / data.totalPages) * 100);
        ...
        if (
          ...
          error.message === "Failed to get crawl status" &&
  ...
  const fetchPartialResults = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/site-audit/partial-results/${crawlId}`,
      );

      if (!response.ok) {
        console.log(
          "Partial results not available yet, status:",
          response.status,
        );
      ...
      const partialCrawlData: CrawlData = {
        ...
        started_at:
          data.started_at || crawlData?.started_at || new Date().toISOString(),
  ...
  const startCrawl = async () => {
    ...
    try {
      ...
      toast({
        title: "Crawl started",
        description:
          "Your site is being crawled with Firecrawl. This may take a few minutes.",
      });
    } catch (err: any) {
      // Typed error for startCrawl
      console.error("Error starting crawl:", err);
      setError(
        err.message || "Failed to start the site audit. Please try again.",
      );
      setIsCrawling(false);
    ...
    try {
      ...
      if (data.status === "completed") {
        ...
        if (finalData.ai_summary_markdown) {
          ...
          console.log(
            "[fetchCrawlResults] No summary in DB for completed crawl, triggering generation for crawlId:",
            crawlId,
          );
      ...
      toast({
        ...
        description:
          err.message || "There was an error retrieving the crawl results.",
        variant: "destructive",
      });
    }
  };
  ...
  const getVisibilityBadge = (score: number) => {
    if (score >= 80) {
      ...
      return (
        <Badge className="bg-[#222222] text-gray-400 border-[#333333]/30">
          None
        </Badge>
      );
    }
  ...
  const getFilteredPages = () => {
    ...
    switch (pageFilter) {
      case "document":
        return crawlData.pages.filter(
          (page: PageDataInCrawl) => page.is_document,
        );
      case "issues":
        return crawlData.pages.filter(
          (page: PageDataInCrawl) => page.issues && page.issues.length > 0,
        );
      case "schema":
        return crawlData.pages.filter(
          (page: PageDataInCrawl) => page.has_schema,
        );
      default:
  ...
  const handleGenerateRecommendations = async (
    ...
    if (!crawlData && !targetCrawlId) {
      // Check if we have data or at least a crawlId to work with
      toast({
        title: "No Data or Crawl ID",
        description:
          "Crawl data or a Crawl ID is not available to generate recommendations.",
        variant: "destructive",
      });
      return;
    }
    if (!targetCrawlId) {
      toast({
        title: "Missing Crawl ID",
        description: "Cannot generate summary without a Crawl ID.",
        variant: "destructive",
      });
      return;
    }
    ...
    const payload = {
      ...
      metricScores: crawlData?.metricScores || {
        aiVisibility: 0,
        contentQuality: 0,
        technical: 0,
        performance: 0,
    ...
    console.log(
      "[DEBUG] Payload sent to /api/site-audit/generate-recommendations:",
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error(
          "[DEBUG] Error data from API route:",
          JSON.stringify(errorData, null, 2),
        );
        throw new Error(
          errorData.error || "Failed to generate AI recommendations",
        );
      }
  ...
  useEffect(() => {
    if (
      ...
      crawlData &&
      crawlId &&
      crawlId !== lastCrawlIdForAiRec
    ) {
      if (crawlData.ai_summary_markdown) {
        // Check if summary is already loaded from DB
        console.log(
          "[AI Auto-Load] Loading summary from DB for crawl:",
          crawlId,
        );
        ...
        console.log(
          "[AI Auto-Gen] Summary not in DB, triggering AI recommendations for new crawl:",
          crawlId,
        );
        handleGenerateRecommendations(crawlId);
    ...
    if (
      ...
      // If crawlId changes and it's not the one we have summary for
      console.log(
        "[AI State Clear] Crawl ID changed, clearing old AI summary/error for crawlId:",
        lastCrawlIdForAiRec,
        "New crawlId:",
        crawlId,
      );
    ...
    if (aiRecommendations && !isGeneratingRecommendations) {
      ...
      const typingInterval = setInterval(() => {
        if (i < textToType.length) {
          ...
          setDisplayedAiRecommendations(
            (prev) => prev + textToType.substring(i, i + chunkSize),
          );
          i += chunkSize;
        } else {
  ...
  const loadHistoricalCrawl = async (histCrawlId: string) => {
    ...
    toast({
      title: "Loading Past Audit...",
      description: "Fetching details for the selected audit.",
    });
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Failed to get historical crawl results",
        );
      }
      let data = await response.json(); // data from /api/site-audit/results endpoint
      console.log(
        "[loadHistoricalCrawl] Data from results API (with potential AI summary):",
      ...
      if (data.status === "completed") {
        ...
        const finalCrawlData: CrawlData = {
          ...
          started_at: data.started_at
            ? safeParseDate(data.started_at)
            : safeParseDate(data.created_at),
          completed_at: data.completed_at
            ? safeParseDate(data.completed_at)
            : safeParseDate(data.created_at),
          pages: data.pages || [],
          screenshots: data.screenshots || [],
          ai_summary_markdown: data.ai_summary_markdown,
        ...
        if (finalCrawlData.ai_summary_markdown) {
          console.log(
            "[loadHistoricalCrawl] AI summary found in historical data for crawlId:",
            histCrawlId,
          );
          ...
          console.log(
            "[loadHistoricalCrawl] No AI summary in historical data, triggering generation for crawlId:",
            histCrawlId,
          );
        ...
        toast({
          title: "Past Audit Loaded",
          description: "Displaying details for the selected audit.",
        });
      } else {
        throw new Error(
          "Selected historical crawl is not in a completed state.",
        );
      }
    } catch (err) {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to load the selected audit.",
      );
      toast({
        ...
        description:
          err instanceof Error
            ? err.message
            : "Could not load the selected audit.",
        variant: "destructive",
      });
  ...
  return (
    <main className="flex flex-1 flex-col gap-8 p-8 overflow-auto bg-[#0c0c0c]">
      ...
      {!isCrawling && !crawlComplete && (
        <div className="flex flex-1 flex-col items-center justify-center p-4 text-center space-y-8 min-h-[calc(100vh-200px)]">
          {" "}
          ...
          <div className="w-full max-w-xl space-y-4">
            ...
            <p className="text-md text-gray-400">
              Enter your website URL to analyze its SEO and AI Engine
              Optimization.
            </p>
            ...
            <div className="flex flex-row gap-2 items-center pt-4">
              ...
              >
                <ArrowRight
                  className={`h-6 w-6 ${validateUrl(siteUrl) ? "text-[#FF914D]" : "text-gray-400"}`}
                />
              </Button>
            </div>
            {error && (
              <p className="text-red-400 text-sm mt-2 text-left pl-8">
                {error}
              </p>
            )}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  ...
                  style={{
                    ...
                    overflowY: "hidden",
                  }} // Prevent DropdownMenuContent itself from scrolling
                  align="start"
                >
                  <DropdownMenuLabel className="text-gray-400 px-3 py-2 text-sm font-medium">
                    Past Audits
                  </DropdownMenuLabel>
                  ...
                  <div className="max-h-80 overflow-y-auto custom-scrollbar">
                    ...
                    {isLoadingHistory && (
                      <DropdownMenuItem
                        disabled
                        className="text-gray-500 px-3 py-2 text-xs focus:bg-transparent cursor-default"
                      >
                        Loading history...
                      </DropdownMenuItem>
                    )}
                              ...
                              <div className="flex flex-col w-full">
                                ...
                                <div className="flex justify-between items-center w-full mt-0.5">
                                  ...
                                  {hist.status !== "completed" && (
                                    <Badge
                                      variant="outline"
                                      className="text-[9px] leading-tight px-1 py-0 border-yellow-700/60 text-yellow-400 bg-yellow-900/30 h-auto"
                                    >
                                      {hist.status}
                                    </Badge>
                                  )}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <p className="text-sm text-gray-400">
                  ...
                  {crawlData &&
                    ` • ${crawlData.crawledPages || 0} pages analyzed`}
                </p>
              </div>
              <Progress
                value={crawlProgress}
              ...
              {crawlData && crawlData.pages && crawlData.pages.length > 0 && (
                <div className="mt-6 pt-4 border-t border-[#222222]">
                  <h3 className="text-sm font-medium text-white mb-3">
                    Live Results ({crawlData.pages.length} pages)
                  </h3>
                  <div className="max-h-[200px] overflow-y-auto rounded-md border border-[#222222]">
                    ...
                    <div className="divide-y divide-[#222222]">
                      {crawlData.pages.map(
                        (page: PageDataInCrawl, index: number) => (
                          <div
                            key={index}
                            className="grid grid-cols-12 gap-4 px-4 py-2 items-center text-xs"
                          >
                            ...
                            <div className="col-span-1">
                              <Badge
                                className={`${getStatusBadgeClass(page.status_code)}`}
                              >
                                {page.status_code}
                              </Badge>
                            </div>
            ...
            <div className="flex-grow space-y-3 md:max-w-[60%]">
              <div>
                ...
                <h2 className="text-2xl font-semibold text-white leading-tight">
                  {crawlData.pages?.find(
                    (p: PageDataInCrawl) => p.url === crawlData.siteUrl,
                  )?.title ||
              ...
              {!isGeneratingRecommendations && displayedAiRecommendations && (
                <div className="relative">
                  ...
                  {displayedAiRecommendations.length > 300 && (
                    <div
                      className={`w-full flex pt-1 ${isAiSummaryExpanded ? "justify-end" : "justify-start"}`}
                    >
                      <Button
                        variant="link"
          ...
          <div className="mt-8">
            ...
            {(() => {
              if (
                ...
                return (
                  <p className="text-gray-500 py-4">
                    No page data available to display issues.
                  </p>
                );
              }
              ...
              if (criticalIssues.length === 0 && warningIssues.length === 0) {
                return (
                  <div className="py-4 text-center">
                    ...
                    <p className="text-sm text-gray-400">
                      Excellent! Your site audit looks clean.
                    </p>
                  </div>
                );
              }
              ...
              const renderIssueList = (
                issues: { pageUrl: string; message: string }[],
              ) => {
                if (issues.length === 0) {
                  return (
                ...
                return (
                  <div className="space-y-1.5 mt-4">
                    {issues.map((issue, index) => (
                      <div
                        key={index}
                        className="p-2.5 bg-[#1A1A1A] border border-[#282828] rounded-md flex justify-between items-center min-h-[50px]"
                      >
                        <p
                          className="text-sm text-gray-400 leading-tight truncate flex-grow mr-4"
                          title={issue.message}
                        >
                          {issue.message}
                        </p>
                        ...
                        >
                          {issue.pageUrl.length > 30
                            ? issue.pageUrl.substring(0, 27) + "..."
                            : issue.pageUrl}
