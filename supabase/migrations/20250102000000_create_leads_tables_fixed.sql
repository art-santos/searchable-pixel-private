...
CREATE TABLE IF NOT EXISTS lead_enrichment_settings (
...
CREATE TABLE IF NOT EXISTS lead_enrichment_usage (
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  date date NOT NULL,
...
CREATE INDEX IF NOT EXISTS user_visits_ip_address_idx ON user_visits(ip_address);
CREATE INDEX IF NOT EXISTS user_visits_enrichment_status_idx ON user_visits(enrichment_status) WHERE enrichment_status = 'pending';
CREATE INDEX IF NOT EXISTS user_visits_utm_source_idx ON user_visits(utm_source) WHERE utm_source IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS leads_ai_attributed_idx ON leads(is_ai_attributed) WHERE is_ai_attributed = true;

CREATE INDEX IF NOT EXISTS contacts_lead_id_idx ON contacts(lead_id);
CREATE INDEX IF NOT EXISTS contacts_email_idx ON contacts(email);
CREATE INDEX IF NOT EXISTS contacts_title_match_score_idx ON contacts(title_match_score DESC);

CREATE INDEX IF NOT EXISTS public_signals_lead_id_idx ON public_signals(lead_id);
